...
//se carga la biblioteca de pagincacion
    ...
    public function articlesByType($id)
    {
        $this->load->model('Model_Article');
        $this->load->library('pagination');

        $config['base_url'] = base_url().'magazine/articlesByType/'.$id."/";
        $config['total_rows'] = count($this->Model_Article->getArticlesByArt_Type($id));
        /*Obtiene el numero de registros a mostrar por pagina */
        $config['per_page'] = 2;
        $config["uri_segment"] = 4;
        /*Se personaliza la paginación para que se adapte a bootstrap*/
        ...
        //$data['articles'] = $this->Model_Article->getArticlesByMagazine($id,$config['per_page'],$desde);
        $data['articles'] = $this->Model_Article->getArticlesByType($id,$config['per_page'],$desde);
...
//Close and output PDF document
    ...
    {
        $this->load->view('header');
        $this->load->view('config/articleonline_view');
        $this->load->view('footer');
    }
